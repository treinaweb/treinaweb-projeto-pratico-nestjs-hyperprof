...
export class ProfessoresService {
  ...
  async update(id: number, updateProfessorDto: ProfessorUpdateDto) {
    try {
      const professor =
        this.professorMapper.toProfessorEntity(updateProfessorDto);
      professor.password = await this.hashSenha(updateProfessorDto.password);
      await this.professorRepository.update(id, professor);
      const professorAtualizado = await this.findOneByEmail(
        updateProfessorDto.email,
      );
  ...
  async updateFoto(professor: Professor, file: Express.MulterS3.File) {
    professor.fotoPerfil = file.location;
